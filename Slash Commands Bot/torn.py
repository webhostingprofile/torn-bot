...
def get_user_stat_history(discord_id, discord_username, days_ago):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            percentage_change['total'] = (total_current - total_previous) / total_previous * 100 if total_previous > 0 else 0
            ...
            stats_details = (
                f"{profile_link}:\n\n"
                f"Old Battle Stats ---> New Battle Stats\n"
                f"Str: {previous_stats['strength']:,} ---> \n {current_stats['strength']:,} ({percentage_change['strength']:.2f}%)\n"
                f"Spd: {previous_stats['speed']:,} ---> \n {current_stats['speed']:,} ({percentage_change['speed']:.2f}%)\n"
...
def format_vitals(data, profile_link):
    ...
    Life
    {vitals['life']}
    Full in {format_time(vitals['life_full'])}

    Energy
    {vitals['energy']}
    Full in {format_time(vitals['energy_full'])}

    Happiness
    {vitals['happiness']}
    Full in {format_time(vitals['happiness_full'])}
